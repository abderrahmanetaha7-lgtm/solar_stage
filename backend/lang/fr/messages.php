<?php

return [

    'invalid_credentials' => 'Email ou mot de passe incorrect',

    'logout_success' => 'Déconnexion réussie',

    'login_success' => 'Connexion réussie',

    'register_success' => 'Compte créé avec succès',

    'password_incorrect' => 'Mot de passe actuel incorrect',

    'current_password_required' => 'Le mot de passe actuel est requis',

    'profile_updated' => 'Profil mis à jour avec succès',

    'account_deleted' => 'Compte supprimé avec succès',

    'product_created' => 'Produit créé avec succès',

    'product_updated' => 'Produit mis à jour avec succès',

    'product_deleted' => 'Produit supprimé avec succès',

    'settings_updated' => 'Paramètres mis à jour avec succès',

    'order_restored' => 'Commande restaurée avec succès',

    'product_out_of_stock' => 'Le produit :product est en rupture de stock',

    'order_created' => 'Commande créée avec succès',

    'order_updated' => 'Commande mise à jour avec succès',

    'invalid_status_transition' => 'Transition invalide de :old vers :new',

    'order_status_updated' => 'Statut de la commande mis à jour avec succès',

    'order_cannot_cancel' => 'Cette commande ne peut pas être annulée',

    'order_cancelled' => 'Commande annulée avec succès',

    'order_archived' => 'Commande archivée avec succès',

    'reset_link_sent' => 'Lien de réinitialisation envoyé par email',

    'reset_link_failed' => 'Impossible d’envoyer le lien de réinitialisation',

    'password_reset_success' => 'Mot de passe réinitialisé avec succès',

    'invalid_token' => 'Token invalide',

    'contact_sent' => 'Message envoyé avec succès',

    'contact_deleted' => 'Message supprimé avec succès',

    'user_deleted' => 'Utilisateur supprimé avec succès',

    'user_updated' => 'Utilisateur mis à jour avec succès',

    'invalid_google_token' => 'Token Google invalide',

    'google_login_success' => 'Connexion avec Google réussie',

];
